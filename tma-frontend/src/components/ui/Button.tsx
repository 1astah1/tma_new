...
export function Button({ variant = 'primary', size = 'md', loading, fullWidth, children, className = '', ...props }: Props) {
  ...
  const variants = {
    primary: 'bg-[var(--tg-button)] text-[var(--tg-button-text)] hover:opacity-90',
    secondary: 'bg-[var(--tg-secondary)] text-[var(--tg-text)] hover:opacity-80',
    outline: 'border-2 border-[var(--tg-button)] text-[var(--tg-button)] bg-transparent',
...
export function Loader({ size = 'md' }: { size?: 'sm' | 'md' | 'lg' }) {
  ...
  return (
    <div className={`${sizes[size]} border-2 border-[var(--tg-button)] border-t-transparent rounded-full animate-spin`} />
  )
}
